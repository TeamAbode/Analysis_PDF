...
def analyze_ab_split(
    df: pd.DataFrame,
    combined_total_col: str,
    variant_col: Optional[str],
) -> dict:
    """
    Produce A/B analysis for the combined-total damages column.

    Returns:
      {
        "ab_present":          True/False,
        "variant_col":         column name (or None),
        "variant_labels":      {"A": "Slider", "B": "open ended"},   # original labels for display
        "overall": {
            "n":      ...,
            "mean":   ...,
            "median": ...,
        ...
        "per_variant": {
            "A": {"n":..., "mean":..., "median":..., "winsor_mean_5_95":..., "raw_label":"Slider"},
            "B": {...},
        },
        "ttest": {
            "t_statistic":   ...,
            "p_value":       ...,
            "diff_means":    ...,
            "interpretation": "<plain-English finding>",
            "significant":    True/False,
        } or None if no A/B present.
      }
    """
    awards = pd.to_numeric(df[combined_total_col], errors="coerce")
    ...
    # Only run t-test if BOTH variants have ≥ 10 non-null awards
